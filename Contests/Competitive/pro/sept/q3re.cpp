...
const int mod = 1e9+7;
int n,x,y,s,t,m,arr[400005],ans[40],lmax[400005], rmax[400005]; 
bool bit[40];
...
void insert(int x, node* p, int pos){
	...
	for(int i=30;i>=0;i--){
		...
		if(i>=1&&bit[i-1]==false) p = p->left;
		else if(i>=1&&bit[i-1]==true) p = p->right;
		else p->posit = pos;
	}
}
int find(int x, node* p){
	...
	for(int i= 30;i>=0;i--){
		if(i>=1){
			if(bit[i-1]==false){
				if(p->right->left==NULL)p = p->left;
				else p = p->right;
			}
			else{
				if(p->left->left==NULL)p = p->right;
				else p = p->left;
			}
		}
		else re = p->posit;
